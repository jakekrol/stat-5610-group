...
![](images/clipboard-4086253343.png){width="643"}
...
![](images/clipboard-3686530741.png){width="643"}
...
1.  Given the training set (seed=116) and the testing set (seed=121), fit KNN on two different values of k.
    ...
    pred_k2 <- knn(train = train_X, test = test_X, cl = train_Y, k = 2)
    pred_k6 <- knn(train = train_X, test = test_X, cl = train_Y, k = 6)
...
misclass_k2 <- mean(pred_k2 != true_Y)
misclass_k6 <- mean(pred_k6 != true_Y)
...
```{r}
x1_range <- seq(min(train_X[,1]), max(train_X[,1]), length.out = 200)
x2_range <- seq(min(train_X[,2]), max(train_X[,2]), length.out = 200)
grid_knn <- expand.grid(X1 = x1_range, X2 = x2_range)

grid$pred_k2 <- knn(train = train_X, test = grid_knn, cl = train_Y, k = 2)
grid$pred_k6 <- knn(train = train_X, test = grid_knn, cl = train_Y, k = 6)

ggplot(grid, aes(x=X1, y=X2, fill=pred_k2)) +
  geom_tile(alpha=0.3) +
  geom_point(data=cbind(train_X, train_Y), aes(x=X1, y=X2, color=factor(train_Y)), size=2) +
  scale_fill_manual(values=c("blue","red"), name="Prediction") +
  scale_color_manual(values=c("blue","red"), name="True Y") +
  labs(title="KNN Decision Boundary (k=2)", x="X1", y="X2") +
  theme_minimal()

# Decision boundary for k=6
ggplot(grid, aes(x=X1, y=X2, fill=pred_k6)) +
  geom_tile(alpha=0.3) +
  geom_point(data=cbind(train_X, train_Y), aes(x=X1, y=X2, color=factor(train_Y)), size=2) +
  scale_fill_manual(values=c("blue","red"), name="Prediction") +
  scale_color_manual(values=c("blue","red"), name="True Y") +
  labs(title="KNN Decision Boundary (k=6)", x="X1", y="X2") +
  theme_minimal()

```

1.  Summarize the Q1, Q2, and Q3 aspects of this "project." Use your imagination. Everyone should have a different scenario.
2.  Think about and discuss with your team some of the bonus questions below.
