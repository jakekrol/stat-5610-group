...
I used KNN to predict a binary disease state based on a diagnostic test result and doctor's belief. Misclassification rates were 0.47 and 0.43 for k=3 and k=7, respectively. Qualitatively, decision boundary plots show that misclassification often occurs near a boundary. Finally, I evaluated a new patient test point with diagnostic test and doctor's belief both at 0.5, KNN predicted a disease state of 1 for both k values. This is bad because X1,X2=0.5 implies that both the test and the doctor were uncertain about the diagnosis, yet the model is forced to pick a definite outcome; in this case, the model chose to diagnose the disease. In my opinion, the misclassification performance being close to a baseline model is not sufficient for real clinical use. Furthermore, the ethical implications of incorrect model predictions on patient health are serious. Therefore, I would not recommend using KNN as a diagnostic tool in this scenario.
...
From the k=8 decision boundary plot, the point X1=0.5, X2=0.5 is in the Y=0 region. 

Concretely,
```{r}
x_0 <- data.frame(X1=0.5, X2=0.5)
y_pred_8 <- knn(
  train= training[, 1:2],
  test= x_0,
  cl= training[, 3],
...
1. Since we found optimal k was 8, we will evaluate misclassification for k = 1, ..., k=16.
...
plot(ks, mcrate, type='b', xlab='k value', ylab='Misclassification Rate', main='KNN Misclassification Rate vs k')
```

2. How is misclassification rate decomposed into variance, bias, and irreducible error for KNN classifier?

The test misclassification rate is the sum of model variance, squared model bias, and irreducible error. Where, irreducible error is the random noise in the data that is unrelated to the predictor variables.

Concretely, high variance implies that the model is sensitive to training data changes, resulting in overfitting.

To show this, we test 3 different training datasets generated with different random seeds, and re-evaluate misclassification rate for k=8 on the same test set.
...
for (s in seeds) {
  ...
  training_s <- cbind(X1_tr,X2_tr,Y_tr)
  y_pred <- knn(train = training_s[,1:2], test = testing[,1:2], 
                       cl = training_s[,3], k = 8)
  misclass_rate <- sum(y_pred != testing[,3]) / n_test
  mcrate_var <- c(mcrate_var, misclass_rate)
}
# do barplot of mcrate for different seeds
barplot(mcrate_var, names.arg = seeds, xlab = "Random Seed", ylab = "Misclassification Rate", main = "KNN Misclassification Rate (k=8) for Different Training Sets")
```

3. What happens to misclassification rate for points outside the unit square?

We generated 3 test sets that extend beyond the unit square, specifically in the range [-10, 10] for both X1 and X2.

```{r}
seeds <- c(1,2,3)
for (s in seeds) {
  n = 100
  X1_te_ext <- runif(n,-10,10)
  X2_te_ext <- runif(n,-10,10)
  Y_te_ext <- rep(0,n)
  for (i in 1:n) {
    Y_te_ext[i] <- generate_y(X1_te_ext[i],X2_te_ext[i])
  }
  testing_ext <- cbind(X1_te_ext,X2_te_ext,Y_te_ext)
  y_pred_ext <- knn(train = training[,1:2], test = testing_ext[,1:2], 
                      cl = training[,3], k = 8)
  misclass_rate_ext <- sum(y_pred_ext != testing_ext[,3]) / nrow(testing_ext)
  print(misclass_rate_ext)
}
```

Unexpectedly, the misclassification rates are low for points outside the unit square, 0.17-0.27 depending on the random seed. Despite KNN being a flexible non-parametric model, it appears to generalize well beyond the training data range in this scenario for the generative model used.

## About team

![](images/clipboard-4086253343.png)

Team ROCstars photo.

### Team semester goals

- Improved mathematical understanding and more practice with regression methods
- Apply statistical learning to real world scenarios
- Gain skills for future data science internships and careers
