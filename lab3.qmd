...
-   I will use 10-fold cross validation to search for the cost and gamma parameters.
...
The best performance of 0.465 misclassification rate was acheived with gamma=0.1 and cost $\in$ {0.1, 1} and (cost=0.1, gamma=10).
...
5.  Make a prediction for a new point (x1, x2) = (0.75, 0.75) for each fitted model. Each individual will have a different point for their predictions. Use the same one you used for Lab1
...
The best cost and gamma parameters were 1 and 0.1, respectively acheiving a misclassification rate of 0.416 under 10-fold cross validation.
...
The radial SVM acheived approximately sensitivity of 0.347, specificity of 0.706, and misclassification of 0.47 on the test set. Although, the radial SVM did not outperform the linear nor degree 2 SVM, we are deciding to use this model instead for the team competition. We ruled out linear because the training/testing data we do have access to is clearly not linearly separable. And, we believe a radial kernel may outperform polynomial kernel too due to its flexibility and the non-linear nature of the data. Radial had the lowest misclassification rate under cross validation and we are hoping this will translate to better performance on the test set. We could acheive lower misclassification rate by tuning directly on the test set, but this would probably severely overfit and not generalize well in the competition.

### Team and individual SVM plot

```{r}
# bar plot mc rate for each model
model_names <- c("Linear SVM", "Degree 2 SVM", "Degree 3 SVM", "Radial SVM (Team)")
mcrates <- c(0.45, 0.45, 0.49, 0.47)
plot_data <- data.frame(Model = model_names, MisclassificationRate = mcrates)
ggplot(plot_data, aes(x=Model, y=MisclassificationRate, fill=Model)) +
  geom_bar(stat="identity") +
  ggtitle("Test Misclassification Rate for Individual and Team SVMs") +
